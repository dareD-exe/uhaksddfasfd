...
const Shayari = () => {
  ...
  useEffect(() => {
    const unsubscribe = listenToShayaris((data) => {
      const sortedShayaris = data.sort((a, b) => b.timestamp - a.timestamp);
      ...
      <div className="space-y-4">
        {shayaris.length === 0 ? (
          ...
          shayaris.map((shayari) => (
            ...
            <div key={shayari.id} className="p-4 bg-white/10 backdrop-blur-md rounded-2xl shadow-lg border border-white/20 relative hover:scale-105 transition-transform duration-300">
  ...
  <span className="svgIcon">
    {copyStatus[shayari.id] ? (
      // Checkmark icon when copied
      <svg fill="white" viewBox="0 0 24 24" height="1em">
        <path d="M20.285 2.857L9 14.143l-5.285-5.286L2 10.571l7 7 12-12z" />
      </svg>
    ) : (
      // Default copy icon
